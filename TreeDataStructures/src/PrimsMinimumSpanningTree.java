...
public class PrimsMinimumSpanningTree {
	...
	public PrimsMinimumSpanningTree(List<List<Edge>> graph) {
	...
	private void solve() {
		...
		visited = new boolean[n]; //know which nodes we have already visited
		mstEdges = new Edge[m]; 
		...
		while (!pq.isEmpty() && edgeCount != m) {
			...
			//and if we have not visited that node,
			//we increase the edgecount and add the edge
			//as well as add the edge cost
		...
		//if the number of edges we went through does not equal m, which is the number of edges n-1, where
		//n is the number of nodes, then mst does not exist.
	...
	private static void example1() {
		...
		PrimsMinimumSpanningTree solver = new PrimsMinimumSpanningTree(g);
